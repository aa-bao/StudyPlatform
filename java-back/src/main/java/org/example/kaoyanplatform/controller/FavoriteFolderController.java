...
public class FavoriteFolderController {
    ...
    @Autowired
    private FavoriteFolderService collectionService;

    // 添加或更新收藏
    @PostMapping("/add")
    @Operation(summary = "添加或更新收藏", description = "收藏题目或更新收藏标签。如果已收藏则更新标签，如果未收藏则新增收藏记录。")
    public Result addCollection(@RequestBody FavoriteFolder collection) {
        // 1. 基础校验
        ...
        wrapper.eq(FavoriteFolder::getUserId, collection.getUserId())
                .eq(FavoriteFolder::getQuestionId, collection.getQuestionId());

        FavoriteFolder exist = collectionService.getOne(wrapper);
        if (exist != null) {
            // 更新标签
    ...
    public Result removeCollection(@RequestBody FavoriteFolder collection) {
        if (collection.getUserId() == null || collection.getQuestionId() == null) {
    ...
    public Result checkCollection(
        ...
        wrapper.eq(FavoriteFolder::getUserId, userId)
                .eq(FavoriteFolder::getQuestionId, questionId);
    ...
    public Result getTags(@Parameter(description = "用户ID", required = true) @RequestParam Long userId) {
        LambdaQueryWrapper<FavoriteFolder> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(FavoriteFolder::getUserId, userId);
        List<FavoriteFolder> list = collectionService.list(wrapper);
