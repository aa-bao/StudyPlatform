...
import org.example.kaoyanplatform.mapper.ErrorQuestionMapper;
 ...
 */
@Service
public class ErrorQuestionServiceImpl extends ServiceImpl<ErrorQuestionMapper, ErrorQuestion> implements ErrorQuestionService {
    ...
    public List<MistakeHeatmapDTO> getMistakeHeatmap() {
        // 1. 获取所有错题记录
        List<ErrorQuestion> allMistakes = list();

        // 2. 先过滤掉没有科目关联的错题记录，再按科目ID分组统计
        Map<Integer, List<ErrorQuestion>> subjectMistakes = allMistakes.stream()
                .filter(mistake -> {
                    // 过滤掉科目ID为null的记录
        ...
        for (Map.Entry<Integer, List<ErrorQuestion>> entry : subjectMistakes.entrySet()) {
            Integer subjectId = entry.getKey();
            List<ErrorQuestion> mistakes = entry.getValue();
    ...
    public List<MistakeHeatmapDTO.HotMistakeQuestion> getHotMistakeQuestions(Integer limit) {
        ...
        Map<Integer, List<ErrorQuestion>> questionMistakes = allMistakes.stream()
                .filter(mistake -> {
                    // 过滤掉科目ID为null的记录
                    Integer subjectId = getSubjectIdByQuestionId(mistake.getQuestionId());
                    return subjectId != null;
                })
                .collect(Collectors.groupingBy(ErrorQuestion::getQuestionId));

        // 构建高频错题列表
        List<MistakeHeatmapDTO.HotMistakeQuestion> hotQuestions = new ArrayList<>();

        for (Map.Entry<Integer, List<ErrorQuestion>> entry : questionMistakes.entrySet()) {
    ...
    private Integer getSubjectIdByQuestionId(Integer questionId) {
        ...
        QuestionSubjectRel relation = mapQuestionSubjectMapper.selectOne(wrapper);
    ...
    private List<MistakeHeatmapDTO.HotMistakeQuestion> getHotQuestionsBySubject(Integer subjectId, Integer limit) {
        ...
        List<QuestionSubjectRel> relations = mapQuestionSubjectMapper.selectList(wrapper);
        ...
        // 获取这些题目的错题记录
        QueryWrapper<ErrorQuestion> mistakeWrapper = new QueryWrapper<>();
        mistakeWrapper.in("question_id", questionIds);
        List<ErrorQuestion> mistakes = list(mistakeWrapper);

        // 按题目ID分组统计
        Map<Integer, List<ErrorQuestion>> questionMistakes = mistakes.stream()
                .collect(Collectors.groupingBy(ErrorQuestion::getQuestionId));
        ...
        for (Map.Entry<Integer, List<ErrorQuestion>> entry : questionMistakes.entrySet()) {
            Integer questionId = entry.getKey();
            List<ErrorQuestion> questionMistakeList = entry.getValue();
